...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("Jenis Penghitungan Bunga: (1 : Flat, 2 : Efektif)");
...
class solver {
    ...
    double anuitas(double i) {
        ...
        double depan = 0;
        depan = (1.0 + (i / 12.0));
        angsuranPerBulan = pinjaman * ((i / 12.0) / (1.0 - (1 / (Math.pow(depan, waktu)))));
        bunga = (pinjaman - anuitasArr()) * (i / 12.0);
        cicilanPokok = angsuranPerBulan - bunga;
        this.anuitas[(int) i] = cicilanPokok;
    ...
    double anuitasArr() {
        double count = 0;
        for (int i = 0; i < this.waktu; i++) {
            count += this.anuitas[i];
        }
        return count;
    }
}
